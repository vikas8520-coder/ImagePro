...
export default function MediaGrid() {
  const { getFilteredItems, selectedIds, viewMode, isImporting } = useMediaStore();
  const items = getFilteredItems();

  // A6: Show skeleton during import
  ...
  if (viewMode === "list") {
    return (
      <div className="space-y-1" role="list" aria-label="Media files list">
        {/* List header */}
        <div
          className="grid grid-cols-[auto_1fr_100px_100px_80px_80px_80px] gap-3 px-3 py-2
